...
func (g *GoGitStorage) CreateRepository(ctx context.Context, repoPath, defaultBranch string) error {
	...
	if _, err := gogit.InitWithOptions(s, nil, gogit.InitOptions{
		...
		// errors.Is, not !=. A wrapped already-exists error compared unequal
		// and was reported as an init failure.
	}); err != nil && !errors.Is(err, gogit.ErrRepositoryAlreadyExists) {
