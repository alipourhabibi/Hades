...
func (s *SQLiteTOTPSecretStorage) Upsert(ctx context.Context, userID, secretEnc string) error {
	...
	_, err := s.q(ctx).ExecContext(ctx,
		`INSERT INTO totp_secrets (user_id, secret_enc, enabled) VALUES (?, ?, 0)
		 ON CONFLICT(user_id) DO UPDATE SET secret_enc = excluded.secret_enc, enabled = 0,
...
func (s *SQLiteTOTPSecretStorage) GetByUserID(ctx context.Context, userID string) (*totpsecret.Row, error) {
	...
	).Scan(&row.ID, &row.UserID, &row.SecretEnc, &row.Enabled, &enrolledAt, &createdAt, &row.LastUsedCounter)
	if err != nil {
		return nil, err
	}
...
func (s *SQLiteTOTPSecretStorage) ConsumeCounter(ctx context.Context, userID string, counter uint64) error {
	res, err := s.q(ctx).ExecContext(ctx,
		`UPDATE totp_secrets SET last_used_counter = ?2
		 WHERE user_id = ?1 AND (last_used_counter IS NULL OR last_used_counter < ?2)`,
		// #nosec G115 -- see the PostgreSQL implementation: unix seconds / 30 cannot overflow int64.
		sqlutil.ID(userID), int64(counter))
	if err != nil {
		return err
	}
